labReport <- function(data, vars, panel, treat, time, times,
                      ...
                      digits=3, append=TRUE, clearPlots=FALSE) {
  ...
  if(length(trlev) != 2) stop('only handles two treatments')

  i <- 0
  nv <- length(vars)
  h <- w <- 4.5*(nv < 7) + 5.5*(nv >= 7 & nv <=9) + 6.5*(nv > 9)

  startPlot(paste('lab-',panel,'%d',sep=''), h=h, w=w)
  omf <- mfrowSet(nv)
  mf <- par('mfrow')
  ...
  for(y in data[vars]) {
    ...
    names(curves) <- c(paste(trlev[1],c('','q1','q3'),sep=''),
  ...
  for(npage in 1:ceiling(nv/prod(mf))) {
    ...
    putFig(panel,paste('lab-',panel,npage,sep=''),
           cap,
           if(npage == 1)
           paste(cap,
                 ...
                 ' and $95^{th}$ quantiles.',
                 ' Vertical bars indicate half-widths of',
  ...
  if(!is.logical(cdf) || cdf) {
    ...
    for(v in data[vrs]) {
      i <- i + 1
      fn <- paste('lab-',panel,'-ecdf-',vrs[i],sep='')
      ...
      putFig(panel, fn,
             ...
             paste('Empirical cumulative distribution function of',
                   label(v),'by treatment over time. \\protect\\treatkey'))
    }
  }
  ...
  if(clearPlots)
    cat('\\clearpage\n', file=paste(panel,'tex',sep='.'), append=TRUE)
